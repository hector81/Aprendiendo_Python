...
while booleano == False: # mientras que el número no sea menor que 1
    ...
    if numero > 0:
        while numero > 0:
            factorial = factorial * numero # multiplicamos el factorial sucesivamente
            # esto es para el formato de la multiplicacion
            if numero != 1:
                multiplicacion_string +=  f"{numero} X "
            else:
                multiplicacion_string +=  f"{numero}"
            numero = numero - 1 # bajamos el contador del numero a 1
